...
let resolve t action ~orphans ?reinstall ~requested request =
   ...
   *     let open Name.Set.Op in
   *     packages_of_names t.remove
   *       (names_of_packages t.remove
   *        -- names_of_packages (Set.of_list request.wish_install)
   *        -- names_of_packages (Set.of_list request.wish_upgrade))
   *   in
