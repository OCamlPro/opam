    ...
    | Some (`set_var | `option as cmd), scope::args ->
      ...
      (match scope with
       ...
       | (`Global | `Switch) as scope ->
         (match cmd, args with
          | `option, [] -> OpamConfigCommand.options_list scope; `Ok ()
          | `option, [var] when not (OpamStd.String.contains_char var '=') ->
            (match scope with
             | `Switch ->
               OpamGlobalState.with_ `Lock_none @@ fun gt ->
               OpamSwitchState.with_ `Lock_read gt @@ fun st ->
               OpamConfigCommand.option_show_switch st var;
               `Ok ()
             | `Global ->
               OpamGlobalState.with_ `Lock_read @@ fun gt ->
               OpamConfigCommand.option_show_global gt var;
               `Ok ())
          | _, (_::_::_) | `set_var, [] ->
